...
export default async function handler(req, res) {
  ...
  try {
    ...
    const gemini = createGeminiProvider({
      ...
      model: 'gemini-2.5-flash',
      generationConfig: {
        maxOutputTokens: 16384, // Máximo para Gemini 2.5 Flash - necessário para JSON com 6 novas seções
        temperature: 0.7,
      }
    });
    ...
    const prompt = createPrompt()
      ...
      .section('NOVAS SEÇÕES - SEMPRE PREENCHA TODAS', [
        ...
        'O objetivo é mostrar ao usuário como ficaria a landing page COMPLETA.',
        '',
        '1. FEATURES (funcionalidades principais):',
        '   features: Array de 4-6 funcionalidades.',
        '   Cada feature: { icon: emoji, title: string (máx 40 chars), description: string (máx 100 chars) }',
        '   Exemplo: [{ icon: "🚀", title: "Rápido e eficiente", description: "Processa seus dados em segundos" }]',
        '   - Foque em COMO o produto funciona tecnicamente',
        '   - Use emojis relevantes ao contexto',
        '',
        '2. BENEFITS (benefícios por categoria):',
        '   benefits: Array de 3-4 categorias de benefícios.',
        '   Cada categoria: { title: string, description: string (máx 80 chars), items: [string, string, ...] }',
        '   Exemplo: [{ title: "Para seu tempo", description: "Economize horas", items: ["50% mais rápido", "Automatiza tarefas"] }]',
        '   - Organize por tipo de ganho (tempo, dinheiro, saúde, etc)',
        '   - 2-4 items por categoria',
        '',
        '3. STATS (estatísticas impressionantes):',
        '   stats: Array de 3-4 métricas.',
        '   Cada stat: { value: string, label: string }',
        '   Exemplo: [{ value: "10k+", label: "Usuários ativos" }, { value: "4.8★", label: "Avaliação média" }]',
        '   - Use números plausíveis mas impressionantes',
        '   - Formato: "X+", "X%", "X★", "X mil"',
        '',
        '4. TESTIMONIALS (depoimentos de clientes):',
        '   testimonials: Array de 3 depoimentos.',
        '   Cada depoimento: { quote: string (máx 150 chars), author: string, role: string }',
        '   Exemplo: [{ quote: "Mudou completamente minha rotina!", author: "Ana Silva", role: "Designer" }]',
        '   - Crie depoimentos críveis e específicos',
        '   - Varie perfis: cliente comum, profissional, autoridade',
        '   - Mencione resultados concretos quando possível',
        '',
        '5. PRICING PLANS (planos de preços):',
        '   pricingPlans: Array de 2-3 planos.',
        '   Cada plano: { name: string, price: string, period: string, features: [string, ...], highlight: boolean }',
        '   Exemplo: [{ name: "Básico", price: "R$ 29", period: "por mês", features: ["Feature 1", "Feature 2"], highlight: false }]',
        '   - Planos: Básico, Premium, (Elite opcional)',
        '   - Preços plausíveis para o mercado brasileiro',
        '   - 3-5 features por plano',
        '   - highlight: true no plano intermediário (mais popular)',
        '',
        '6. GUARANTEE (garantia de satisfação):',
        '   guarantee: { badge: emoji, title: string, description: string (máx 120 chars) }',
        '   Exemplo: { badge: "✓", title: "Garantia de 30 dias", description: "Se não gostar, devolvemos 100% do seu dinheiro" }',
        '   - Use garantia padrão de 7-30 dias',
        '   - Reduza risco percebido',
        '',
        '⚠️ IMPORTANTE: Se o brief for vago (ex: "app de meditação"), CRIE conteúdo genérico mas relevante.',
        'Exemplo para "app de meditação":',
        '  - features: ["🧘 Meditações guiadas", "🎵 Sons relaxantes", "📊 Progresso"]',
        '  - stats: ["10k+ usuários", "4.8★ avaliação", "85% menos ansiedade"]',
        '  - testimonials: 3 depoimentos fictícios sobre redução de estresse',
        '  - pricing: R$ 29/mês (básico), R$ 79/mês (premium)',
        '  - guarantee: "7 dias grátis - cancele quando quiser"',
      ])
      .section('FORMATO JSON OBRIGATÓRIO', [
        ...
        '  ],',
        '  "benefits": [',
        '    { "title": "string", "description": "string", "items": ["item1", "item2"] }',
        '  ],',
        '  "stats": [',
        ...
        '  ],',
        '  "pricingPlans": [',
        '    { "name": "string", "price": "string", "period": "string", "features": ["f1", "f2"], "highlight": boolean }',
        '  ],',
        '  "guarantee": { "badge": "emoji", "title": "string", "description": "string" },',
        '  "faq_items": [',
